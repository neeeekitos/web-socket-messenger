...
export default function Messenger(props) {
    ...
    const connect = () => {
        ...
        client.configure({
            brokerURL: 'ws://localhost:8082/stomp2',
            onConnect: async () => {
    ...
    const getMessagesByChatId = (chatId) => {
        axios.post(`http://localhost:8082/actions/changeCurrentChat/${chatId}`, null, {
            headers: {
            ...
            axios.get(`http://localhost:8082/messages/getMessagesByChatId?chatId=${chatId}`).then(console.log);
    ...
    const isAuthenticated = async () => {
        const response = await axios.get("http://localhost:8082/auth/isAuthenticated");
        const isAuth = response.data;
        console.log(isAuth ? "User is authenticated" : "User is not authenticated");
        if (isAuth) {
            setAuth(true);
            const usernameResponse = await axios.get("http://localhost:8082/auth/getUsername");
    ...
    const authenticate = async () => {
        const response = await axios.post('http://localhost:8082/auth/authenticate', username,
            {
                headers: {
    ...
    const afterAuth = async () => {
        ...
        await axios.get("http://localhost:8082/actions/allUsers");
        await axios.get("http://localhost:8082/actions/allUserChats");
    }
    ...
    const onAllChatsReceived = (message) => {
        ...
        if (message.body) {
            ...
            if (chatsResponse.length > 0) {
                ...
                // axios.get(`http://localhost:8082/messages/getMessagesByChatId?chatId=${currentChatId}`).then();
            }
    ...
    const sendMessage = (message) => {
        ...
        axios.post("http://localhost:8082/messages/sendMessage", message, {
            headers: {
                'Content-Type': 'text/plain'
            },
            responseType: 'text'
